...
# Giả sử bạn đã có tập dữ liệu X
# X là một mảng 2D, mỗi hàng là một điểm dữ liệu và mỗi cột là một đặc trưng

# Khởi tạo mô hình K-means với số cụm (clusters) mong muốn
k = 2  # Số cụm
kmeans_model = KMeans(n_clusters=k, n_init=10) # n_init số lần chạy
...
# Tìm ra tâm của các cụm
centroids = kmeans_model.cluster_centers_
# In ra nhãn của các điểm dữ liệu
print("Labels:", labels)
...
Y_preds = kmeans_model.predict(X_test)
...
scores = cross_val_score(kmeans_model,X, Y, cv=10)
...
y_pred1 = kmeans_model.predict([x_real1])
...
y_pred2 = kmeans_model.predict([x_real2])
...
y_pred3 = kmeans_model.predict([x_real3])
